...
public class EnemyAI : MonoBehaviour
{
    ...
    public class EnemyHealth
    {
        ...
        {
            public Transform playerCamera; //the camera set to follow the player (automatically assigns to Camera.main is not set)
    ...
    {
        public bool allowRespawn; //determines whether or not the enemy can respawn after being killed
    ...
    void MovingEnemy()
    {
        anim.SetFloat("MovementSpeed", Mathf.Abs(movementSpeed2));
    ...
    void EnemyDeath()
    {
        ...
        if (hitCount >= maximumHealth)
        {
            if (!dead)
            {
                if (health.hurtEffect != null)
                {
                    Instantiate(health.hurtEffect, transform.position, transform.rotation);
                    anim.SetTrigger("damage");
                }
                if (health.deathEffect != null)
                {
                    //Instantiate(health.deathEffect, transform.position, transform.rotation);
            ...
            else
            {
                ...
                if (GetComponent<Renderer>() && GetComponent<Renderer>().enabled)
                {
                    rendererOnOff = true;
                ...
                if (dist <= damage.damageRadius && distY <= 7 && hitCount < maximumHealth && lastHitCount == hitCount)
                {
                    if (playerAngle < damage.playerViewpointEnemyMustBeInToGetHit)
                    {
                        ...
                        if (player.GetComponent<PlayerController>() != null && player.GetComponent<PlayerController>().enabled)
                        {
                            ...
                            {
                                hitCount += player.GetComponent<PlayerController>().attacking.crouchAttackStrength;
                            }
                        }
                        else
                        {
